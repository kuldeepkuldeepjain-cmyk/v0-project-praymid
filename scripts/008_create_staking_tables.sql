-- Create staking_coins table (available cryptocurrencies for staking)
...
-- Create stakes table (participant staking records)
CREATE TABLE IF NOT EXISTS stakes (
  ...
  status VARCHAR(50) NOT NULL DEFAULT 'active', -- active, claimed, restaked
...
-- Create staking_rewards table (daily accrued rewards)
...
-- Create staking_claims table (claim history)
CREATE TABLE IF NOT EXISTS staking_claims (
  ...
  claim_type VARCHAR(50) NOT NULL DEFAULT 'claim', -- claim, restake
...
CREATE INDEX IF NOT EXISTS idx_stakes_participant_id ON stakes(participant_id);
CREATE INDEX IF NOT EXISTS idx_stakes_participant_email ON stakes(participant_email);
CREATE INDEX IF NOT EXISTS idx_stakes_coin_symbol ON stakes(coin_symbol);
...
CREATE INDEX IF NOT EXISTS idx_staking_rewards_participant_id ON staking_rewards(participant_id);
CREATE INDEX IF NOT EXISTS idx_staking_rewards_claimed ON staking_rewards(claimed);
CREATE INDEX IF NOT EXISTS idx_staking_claims_stake_id ON staking_claims(stake_id);
CREATE INDEX IF NOT EXISTS idx_staking_claims_participant_id ON staking_claims(participant_id);
CREATE INDEX IF NOT EXISTS idx_staking_coins_symbol ON staking_coins(coin_symbol);

-- Insert default staking coins
INSERT INTO staking_coins (coin_symbol, coin_name, apy, risk_level, description) VALUES
