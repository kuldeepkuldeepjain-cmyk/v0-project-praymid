...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const oldBalance = Number(participant.account_balance || 0)
    ...
    // Log transaction
    await db.query(
      "INSERT INTO transactions (participant_id, participant_email, type, amount, description, balance_before, balance_after, status) VALUES ($1,$2,'credit',$3,$4,$5,$6,'completed')",
      [participant.id, email.toLowerCase().trim(), Number(amount), `Admin manual credit of $${amount}`, oldBalance, newBalance]
    ).catch(() => {})
    await db.query(
