...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // If approved, update participant's activation status and add $180 to account
    if (action === "approve") {
      // Get current balance
      ...
      const newBalance = Number(currentBalance) + 180
      ...
      if (payment.participant_email) {
        await supabase.from("notifications").insert({
          ...
          message: "Your contribution has been approved. $180 has been credited to your account. Next contribution available after 30 days.",
          read_status: false,
        })
      }
    } else {
