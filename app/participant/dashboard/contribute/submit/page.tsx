...
export default function SubmitContributionPage() {
  ...
  const checkPendingSubmission = async () => {
    ...
    try {
      const res = await fetch(`/api/participant/contributions/status?email=${encodeURIComponent(participantData?.email)}`)
      const data = await res.json()
      setHasPendingSubmission(data.hasPending || false)
    } catch (err) {
